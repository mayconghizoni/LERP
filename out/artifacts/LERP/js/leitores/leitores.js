...
$(document).ready(function () {
    ...
    LERP.leitores.exibir = function(listaLeitores) {
        ...
        if(listaLeitores != undefined && listaLeitores.length > 0 ) {
            ...
            for (var i = 0; i < listaLeitores.length; i++) {
                ...
                if(listaLeitores[i].status == 1 || listaLeitores[i].status == 2){
                    leitor += "<div class=\"card livros\">" +
                        ...
                        "<button type=\"button\" href=\"#\" class=\"btn btn-margin btn-outline-secondary\" onclick=\"LERP.leitores.inativar('"+listaLeitores[i].id+"')\">Inativar leitor</button>" +
                        "</div>" +
                        "</div>"
                }
    ...
    LERP.leitores.inativar = function (id) {

        let modalInativarLeitor = {
            ...
            buttons:{
                "Sim": function () {
                    ...
                    $.ajax({
                        type: "PUT",
                        url: "/LERP/rest/leitor/inativar/"+id,
                        success: function (msg) {
                            LERP.modalAviso(msg);
                            $("#modalInativarLeitor").dialog("close");
                            LERP.leitores.buscar();
                        },
                        error: function (info) {
        ...
        $("#modalInativarLeitor").dialog(modalInativarLeitor);
